...
export class SortableComponent implements ControlValueAccessor {
    ...
    /** Used to disable dropping on the sortable group */
    @Input() dropDisable = false;

    /** Used to disable sorting on the sortable group */
    @Input() sortDisable = false;
    ...
    onItemDragover(event: DragEvent, i: number): void {
        if (!this.transfer.getItem() || this.dropDisable || this.sortDisable) {
            return;
        }
    ...
    cancelEvent(event: DragEvent): void {
        if (!this.transfer.getItem() || !event || this.dropDisable || this.sortDisable) {
            return;
        }
        event.preventDefault();
    }
    ...
    onDrop(item: DraggableItem): void {
        if (this.dropDisable && this.sortDisable) {
            return;
        }
